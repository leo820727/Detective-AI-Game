...
class DetectiveEngine:
    ...
    def generate_characters(self, background: Background, mystery: MysteryLogic, count=4):
        ...
        故事背景（包含核心物證）：{mystery.full_story}
        ...
        4. 每位角色的動機和性格必須與他們在【真相】裡的行為一致。**🚨 煙霧彈要求**：即使是無辜的角色，也必須在 `secret_motive` 欄位設計屬於他們自己的破事或陰暗秘密（例如盜用公款、地下情、被勒索等），並在 `initial_alibi` 欄位給出案發時的行蹤。讓他們因為想掩護自己的秘密而顯得心虛可疑，絕不可讓他們只是單純指認真凶的目擊者。
        5. **嫌疑事由 (suspicion_reason)**：解釋為什麼警方會懷疑他。如果他在故事背景中有「異常行為」，請設定這其實是為了掩飾他的 `secret_motive`，而不是跟真凶有關！
    ...
    def get_dynamic_response(self, character: Character, question: str, mystery: MysteryLogic):
        """實時生成嫌疑人的回應與後續追問，利用資訊隔離牆讓無辜者無法暴雷"""
        
        # 資訊隔離：只有真凶知道真相，其他人只知道自己的秘密和公開的故事
        if character.is_killer:
            context_section = f"【上帝視角：案件真相全貌】：{mystery.truth_reveal_story}"
        else:
            context_section = f"【已知案件背景】：{mystery.full_story}\n【警方會懷疑你的原因】：{character.suspicion_reason}\n【你案發時的行蹤 (alibi)】：{character.initial_alibi}\n【你自己的不可告人秘密 (secret_motive)】：{character.secret_motive}\n*(請注意：這是一個獨立的秘密，跟殺死死者的真凶無關！)*"
            
        prompt = f"""扮演偵探遊戲中的嫌疑人。
        你的身分：{character.name} ({character.role})
        性格：{character.personality}
        關係：{character.relation_to_victim}
        你是真凶嗎：{"是，你是殺死死者的真凶！但你必須強力隱瞞真相，試著為自己開脫，只能在對話中稍微留下一點邏輯破綻。" if character.is_killer else "不是！你是一個無辜者，被捲入這場命案中。"}
        ...
        回應規則（極度重要🚨）：
        1. {"你必須掩飾自己犯案的真相，盡量為自己開脫、撒謊或把嫌疑推給別人。" if character.is_killer else "做為被捲入命案的無辜者，你【完全不知道】是誰殺了死者。你滿腦子只擔心自己的「不可告人秘密」暴露！在對答中，請極力掩飾你自己的秘密，為自己的行蹤辯護，甚至可以為了自保去隨意懷疑別人。請生動地描述你的辯詞，絕對不要一直回答「不知道」。"}
        2. 你的回應必須符合角色的知識範圍。
        3. 回應長度控制在 50~100 字之間，展現戲劇張力。
        4. **後續追問選項**：這 2 個選項必須與剛才的「回應內容」產生合理的邏輯連貫。請發揮創意引導偵探，但不要隨意捏造完全無關的荒謬線索。"""
