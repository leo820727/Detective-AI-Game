...
class DetectiveEngine:
    ...
    def generate_mystery(self, background: Background, count=4):
        ...
        3. **真相還原 (truth_reveal_story)**
           - **🚨 劇情要求**：兇手的動機必須具有「反轉」或「深層心理」元素（例如：悲劇性的誤會、扭曲的信仰、為了保護他人的意外等）。絕對禁止使用老套的動機，如爭奪遺產、單純的見財起意或情殺。
    ...
    def generate_characters(self, background: Background, mystery: MysteryLogic, count=4):
        ...
        6. **初始詢問 (initial_questions)**：提供 2 個偵探對他進行第一次偵訊的問題。
           - **🚨 嚴格限制**：問題的內容必須「完全且僅限於」上面【故事背景】與【關鍵證據】中實際出現過的物品和行為。絕對禁止自己發明或添加故事以外的道具或情節。
    ...
    def get_dynamic_response(self, character: Character, question: str, mystery: MysteryLogic):
        ...
        4. 【🚨 後續追問選項限制】：這 2 個追問選項必須嚴格扣緊你剛才的「回應內容」或場景中的「核心物證」。絕對禁止自己發明或添加劇本裡沒有提及的物品或情節！"""
